...
object alignPaths extends ((Set[Declarative], IEngine) => Set[PathAlignment]) {
  ...
  def apply(assumption: Set[Declarative], ie: IEngine) = {
    ...
    for {
      ...
      if logicallyRelated(ie, clueTerms, sub, sup)
      ...
      log = logicallyAligned(ie, clueTerms, subPath, supPath)
      if log != "none"
      if signMatch(subPath, supPath)
      if !headStopWord(subPath)
      if !headNamedEntity(supPath)
    } yield {
      PathAlignment(subPath, supPath, (log == "soft"))
    }
  }
  ...
  private[this] def logicallyAligned(ie: IEngine, clueTerms: Set[Term], subPath: Path, supPath: Path) = {
    ...
    if (!(aRefClue1 intersect bRefClue1).isEmpty || !(aRefClue2 intersect bRefClue2).isEmpty) {
      ...
      val endAlign = an.rseq.exists(ar => bn.rseq.exists(br => {
      ...
      if (endAlign) {
        "soft"
      } else if (an.children.isEmpty && bn.children.isEmpty) {
        "hard"
      } else {
        "none"
      }

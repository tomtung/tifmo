...
import extension.{SelNum, SelSup}
import document.{Token, Document, tentRootNeg, tentRoles, tentRoleOrder}
...
object parse extends ((String, String) => (Document, Document)) {
  ...
  private[this] def makeDocument(anno: Annotation, id: String) = {
    ...
    val tokens = for {
      ...
      val lemma = if (ner == "DATE" || ner == "TIME") {
        ...
        EnWordNet.stem(atoken.get(classOf[LemmaAnnotation]), pos)
      }
      ...
      ret.word = EnWord(lemma, mypos, ner)
  ...
  private[this] def makeDCSTrees(anno: Annotation, doc: Document) {
    ...
    for (sentence <- anno.get(classOf[SentencesAnnotation])) {
      ...
      edges = {
        var ret = edges
        for ((ptk, rel, spc, ctk) <- edges; if rel.matches("[nc]?subj") && ptk._2.lemma == "be" && ptk._3.matches("VB.*")) {
          ret = ret - ((ptk, rel, spc, ctk))
          for ((pp, rrel, sspc, cc) <- edges; if pp == ptk && !rrel.matches("[nc]?subj")) {
            ret = ret - ((pp, rrel, sspc, cc)) + ((ctk, rrel, sspc, cc))
          }
        ...
        def scan(i: Int) {
          if (i < finish) {
            ...
            def sameNE(tk: Token) = {
              ...
              tkword.ner == theword.ner &&
                (theword.mypos != "D" || tkword.lemma == theword.lemma)
            }
            if (theword.ner != "O") {
              val tmpmax = {
                ...
                var cachemax = i
                def loop() {
                  for ((ptk, rel, spc, ctk) <- ret; if rel != "conj") {
        ...
        rel match {
          ...
          case "amod" => {
            if (ptk._2.mypos == "N" && ctk._3 == "JJS") {
              doc.getTokenNode(ptk._1).selection = SelSup(ctk._2.lemma, ARG)
            } else {
              doc.getTokenNode(ctk._1).outRole = ARG
              doc.getTokenNode(ptk._1).addChild(MOD, doc.getTokenNode(ctk._1))
            }
          ...
          case "nn" => {
            doc.getTokenNode(ctk._1).outRole = ARG
            val tmp = if (ctk._2.mypos == "D") TIME else ARG
            doc.getTokenNode(ptk._1).addChild(tmp, doc.getTokenNode(ctk._1))
          }
          case "npadvmod" => {
            doc.getTokenNode(ctk._1).outRole = ARG
